...
class WatchlistDAOTest {
    ...
    void init() throws SQLException {
        // initialize DAO (will create table if needed) and grab connection
    ...
    void cleanTable() throws SQLException {
        // clear any existing watchlist entries so each test starts fresh
    ...
    void addForUser_and_list() throws SQLException {
        ...
        assertTrue(list.contains(StockName.WES_AX), "List must contain APPLE");
    }
    ...
    void addForUser_idempotent() throws SQLException {
        ...
        watchlistDAO.addForUser(TEST_USER_ID, StockName.AMC_AX);  // no-op
        List<StockName> list = watchlistDAO.listForUser(TEST_USER_ID);

        assertEquals(1, list.size(), "Duplicate insert should be ignored");
        assertEquals(StockName.AMC_AX, list.get(0));
    }
